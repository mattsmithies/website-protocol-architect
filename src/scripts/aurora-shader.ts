// aurora-shader.ts — Raw WebGL fragment shader
// Pure flowing chaos backdrop. The force graph provides the structure.
...
const FRAGMENT_SHADER = `
  ...
  void main() {
    ...
    color = mix(color, violet * 0.6, smoothstep(0.1, 0.8, n1) * 0.18);
    color = mix(color, sky_blue * 0.5, smoothstep(0.0, 0.7, n2) * 0.1);
    color = mix(color, indigo * 0.4, smoothstep(0.2, 0.9, n1 * n2) * 0.08);
    color += violet * smoothstep(0.5, 0.9, n3) * 0.04;
...
export function initAuroraShader(canvas: HTMLCanvasElement): boolean {
